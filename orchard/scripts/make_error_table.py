...
import pandas as pd
import yaml
from ase.data import atomic_numbers, chemical_symbols, ground_state_magnetic_moments
...
def error_table3(dirs, Analyzer, models, rows, basis, functional):
    ...
    for d in dirs:
        ...
        fx_total_ref_true = 0
        for Z in list(formula.keys()):
            fx_total_ref_true += formula[Z] * predict_total_exchange_unrestricted(
                element_analyzers[Z]
            )
        ...
        for i, model in enumerate(models):
            fx_total_ref = 0
            for Z in list(formula.keys()):
                fx_total_ref += formula[Z] * predict_total_exchange_unrestricted(
                    element_analyzers[Z], model=model
                )
            xef_pred, eps_pred, neps_pred, fx_total_pred = predict_exchange(
                analyzer, model=model
            )
            print(fx_total_pred, fx_total_true, fx_total_ref, fx_total_ref_true)
            print(
                fx_total_pred - fx_total_true,
                fx_total_pred - fx_total_true - (fx_total_ref - fx_total_ref_true),
            )

            ise[i] += np.dot(
                (eps_pred[condition] - eps_true[condition]) ** 2, weights[condition]
            )
            tse[i] += ((eps_pred[condition] - eps_true[condition]) ** 2).sum()
            rise[i] += np.dot(
                (xef_pred[condition] - xef_true[condition]) ** 2, weights[condition]
            )
    ...
    errtbl = np.array(
        [ae_fx_total_rmse, fx_total_rmse, rmise, rmse, rrmise, rrmse]
    ).transpose()

    return (fxlst_true, fxlst_pred, errlst, ae_errlst), (columns, rows, errtbl)
...
def error_table3u(dirs, Analyzer, models, rows, basis, functional):
    ...
    for d in dirs:
        ...
        for i, model in enumerate(models):
            ...
            fx_total_pred = predict_total_exchange_unrestricted(analyzer, model=model)
            print(fx_total_pred, fx_total_true, fx_total_ref, fx_total_ref_true)
            print(
                fx_total_pred - fx_total_true,
                fx_total_pred - fx_total_true - (fx_total_ref - fx_total_ref_true),
...
def error_table_rxn(mol_ids, fname_base, models, formulas=None, extra_dirs=None):
        ...
        if args.extra_dirs is None:
            ...
            for dd in ddirs:
                ...
                raise FileNotFoundError(
                    "Could not find dataset in provided dirs: {}".format(fname)
                )
        return fname
    ...
    return (
        ...
        rxn_names,
        np.array(rxn_diffs),
        np.array(noise_factors, dtype=np.float64),
    )
    ...
    for d in dirs:
        ...
        for Z in list(formula.keys()):
            ...
            _, _, fx_total_ref_tmp = predict_correlation(
                element_analyzers[Z], restricted=restricted
            )
            fx_total_ref_true += formula[Z] * fx_total_ref_tmp
        eps_true, neps_true, fx_total_true = predict_correlation(analyzer)
        fxlst_true.append(fx_total_true)
        ae_fxlst_true.append(fx_total_true - fx_total_ref_true)
        count += eps_true.shape[0]
        for i, model in enumerate(models):
            ...
            for Z in list(formula.keys()):
                restricted = (
                    True if type(element_analyzers[Z]) == CCSDAnalyzer else False
                )
                _, _, fx_total_tmp = predict_correlation(
                    element_analyzers[Z], model=model, restricted=restricted
                )
                fx_total_ref += formula[Z] * fx_total_tmp
            eps_pred, neps_pred, fx_total_pred = predict_correlation(
                analyzer, model=model
            )
            print(
                fx_total_pred - fx_total_true,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base-sysdir", type=str, default=None, help="If provided, append to mol_files"
    )
    ...
    for mol_id in mol_ids:
        dirs.append(get_save_dir(SAVE_ROOT, "KS", args.basis, mol_id, args.functional))
    ...
    if args.xsuffix is not None:
        ...
        res1, res2 = error_table3(
            dirs, Analyzer, models, rows, args.basis, args.functional
        )

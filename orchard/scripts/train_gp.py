...
import numpy as np
import yaml
from ciderpress.data import filter_descriptors, load_descriptors
...
def parse_dataset(args, i, val=False):
    ...
    X, y, rho, rho_data = filter_descriptors(X, y, rho_data, tol=args.density_cutoff)
    print(X.shape, n)
    if args.randomize:
...
def main():
    ...
    parser.add_argument(
        "feature_file", type=str, help="serialized FeatureList object in yaml format"
    )
    parser.add_argument(
        "datasets_list",
        nargs="+",
    ...
    parser.add_argument(
        "--tail", action="store_true", help="atomic tail exact constraint"
    )
    parser.add_argument(
        "-o",
        "--desc-order",
    ...
    if args.length_scale is not None:
        args.length_scale = np.array(
            args.length_scale
        )  # parse_list(args.length_scale, T=float)
    if args.agpr_scale is not None:
        args.agpr_scale = np.array(
            args.agpr_scale
        )  # parse_list(args.agpr_scale, T=float)
    ...
    gpr.fit(X, y, add_heg=args.heg, add_tail=args.tail, optimize_theta=True)

...
from interpolation.splines import UCGrid, eval_cubic, filter_cubic
...
def get_mapped_gp_evaluator_simple(gpr, rbf_density=8, max_ngrid=120):
    ...
    if len(inds) == 0:
        ...
        funcps.append(fps)
        spline_grids.append(UCGrid(dims[inds[0]], dims[inds[1]], dims[inds[2]]))
        ind_sets.append(tuple(inds))
    elif len(inds) == 4:
        k = np.einsum(
            "ni,nj,nk,nl->nijkl", k0s[inds[0]], k0s[inds[1]], k0s[inds[2]], k0s[inds[3]]
        )
        funcps.append(np.einsum("n,nijkl->ijkl", alpha, k))
        spline_grids.append(
            UCGrid(dims[inds[0]], dims[inds[1]], dims[inds[2]], dims[inds[3]])
        )
        ind_sets.append(tuple(inds))
    else:
...
def get_mapped_gp_evaluator(
    ...
    if n == 1 and order == 2:
        ...
        tsp = eval_cubic(spline_grids[0], coeff_sets[0], X[:, 1:2])
        ...
        print(
            np.mean(
                np.abs(
                    res - evaluator.predict_from_desc(D, max_order=2, vec_eval=True)[0]
                )
            )
        )
        ...
        print(
            np.mean(
                np.abs(
                    res - evaluator.predict_from_desc(D, max_order=3, vec_eval=True)[0]
                )
            )
        )
...
def main():
    ...
    parser.add_argument(
        "--basis", default="def2-qzvppd", type=str, help="basis set code"
    )
    parser.add_argument(
        "--functional",
        metavar="functional",
    ...
    gpr = load(args.fname)
    assert (gpr.args.validation_set is None) or len(
        gpr.args.validation_set
    ) % 2 == 0, "Need pairs of entries for datasets list."
    ...
    for i in range(1, nv):
        (
            ...
            rhon,
            rho_datan,
        ) = parse_dataset(gpr.args, i, val=True)
    ...
    if not gpr.args.agpr:
        ...
        evaluator = get_mapped_gp_evaluator(
            gpr, srbf_density=args.srbfd, arbf_density=args.arbfd, max_ngrid=args.maxng
        )
    else:
        evaluator = get_mapped_gp_evaluator(
            gpr,

...
from ciderpress.models.jax_pw6b95 import (
    PW6B95_DEFAULT_PARAMS,
    PW8B95_DEFAULT_PARAMS,
    PW11B95_DEFAULT_PARAMS,
    PW12B95_DEFAULT_PARAMS,
    PW13B95_DEFAULT_PARAMS,
    PW14B95_DEFAULT_PARAMS,
    build_xcfunc_and_param_grad,
    pw6b95_train,
    pw8b95,
    pw11b95,
    pw12b95,
...
def load_molecular_data(basis, functional, mol_id, d4_functional=None):
    ...
    eb, exc, etot = (
        analyzer.get("e_base"),
        analyzer.get("exc_orig"),
        analyzer.get("e_tot_orig"),
    )
    ...
    for rxn_id, rp in rxn_predictions.items():
        ...
        for i in range(nlinp):
            ...
            for j in range(nlinp):
                cov[i, j] += (
                    rp["weight"] * rp["de_pred"][linp[i]] * rp["de_pred"][linp[j]]
                )
...
def train_gd(
    vpg_func, inputs_dict, formulas, init_params, pweights, niter=10, tol=1e-3, rr=1e-3
):
    """
    ...
    for iter_num in range(niter):
        ...
        rxn_predictions = compute_rxn_preds_and_derivs(
            formulas, mol_predictions, pnames
        )
...
def train_bfgs(
    ...
    res = minimize(
        ...
        method=train_method,
        jac=True,
        options={"gtol": 1e-7, "maxfun": 300, "maxiter": 300},
    )
    if mul_pweights:
...
def main():
    ...
    parser.add_argument("--xc-model-name", type=str, default="PW6B95", help=xcdesc)
    ...
    parser.add_argument(
        "--relative-train-rate", type=float, default=1.0, help="Relative training rate"
    )
    parser.add_argument(
        "--param-weights-file",
        type=str,
    ...
    parser.add_argument(
        "--niter", type=int, default=10, help="Maximum number of iterations"
    )
    parser.add_argument(
        "--rtol",
        type=float,
    ...
    for mol_id in mol_ids:
        mol_data[mol_id] = load_molecular_data(
            args.basis, args.functional, mol_id, d4_functional=args.d4_functional
        )
